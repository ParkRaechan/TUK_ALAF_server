...
- **How?** DB에 10,000건의 더미 데이터를 삽입하여 부하 테스트를 진행했습니다. 전체 데이터 로드 시 224ms까지 치솟았던 초기 응답 시간(TTFB)을, `LIMIT`과 `OFFSET`을 활용한 **백엔드 페이징(Pagination) 및 프론트엔드 스크롤(더보기) 아키텍처**로 개편하여 **10~20ms 수준으로 대폭 단축**시켰습니다.
